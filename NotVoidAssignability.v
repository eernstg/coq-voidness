...
Inductive ClassTypeNamed : Name -> NameDartTypes -> (Name * DartTypes) -> Prop :=
| ctn_first : ∀ name args ctypes,
  ClassTypeNamed name (ndts_cons (name, args) ctypes) (name, args)
| ctn_rest : ∀ name1 ctype ctypes name2 args2,
  name1 <> name2 ->
  ClassTypeNamed name1 ctypes ctype ->
  ClassTypeNamed name1 (ndts_cons (name2, args2) ctypes) ctype.
...
with NotVoidAssignablePairwise : DartTypes -> DartTypes -> Prop :=
| nvap_cons_first : ∀ dt1 dts1 dt2 dts2,
  NotVoidAssignable dt1 dt2 ->
  NotVoidAssignablePairwise (dts_cons dt1 dts1) (dts_cons dt2 dts2)
...
with NotVoidAssignableClassTypes : NameDartTypes -> NameDartTypes -> Prop :=
| nvact_upcast : ∀ name args1 ctypes1 args2 ctypes2,
  DartSubtypesClassTypes ctypes1 (ndts_cons (name, args2) ctypes2) -> (* This is an upcast, to (name, args2) :: ctypes2 *)
  ...
  NotVoidAssignableClassTypes ctypes1 (ndts_cons (name, args2) ctypes2)
| nvact_downcast : ∀ name args1 ctypes1 args2 ctypes2,
  DartSubtypesClassTypes ctypes2 (ndts_cons (name, args1) ctypes1) -> (* This is a downcast, to ctypes2 *)
  ClassTypeNamed name ctypes2 (name, args2) -> (* In ctypes2, the type named name is (name, args2) *)
  NotVoidAssignablePairwise args1 args2 -> (* At least one type argument is not-void-assignable *)
  NotVoidAssignableClassTypes (ndts_cons (name, args1) ctypes1) ctypes2.
